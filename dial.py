...
if(test == 0):  # change 1 to 0 once done
    ...
    # time, gx, gy, gz, ax, ay, az, p, y, r, tmp, pressure, alt, alt gps, lat, lon, sat, speed, pps
    f.write("time\tgx\tgy\tgz\tax\tay\taz\tp\ty\tr\ttmp\tpressure\talt\tlat\tlon\talt\tlat\talt gps\tsat\tspeed\tpps\n")
    f.close()



# Initialise screen.
screen = pygame.display.set_mode((900, 300), pygame.FULLSCREEN)
screen.fill(0x000)

   
# Initialise Dials.
horizon = Horizon(10,10, 250, 250)
rfSignal = RfSignal(10,280,500,150)
dist = GenericText(300,55,25)
pps = GenericText(300,110,25)
speed = GenericText(300,165,25)
alt = GenericText(300,220,25)
...
while 1:
   ...
   if(len(rf_data) >= 19):
      ...
      # Update dials.
      try:
          horizon.update(screen, -float(rf_data[7]), -float(rf_data[8]))
          dist.update(screen, "Distance: ", float(rf_data[2]), 3)
          pps.update(screen, "PPS: ", float(rf_data[18]), 3)
          speed.update(screen, "Speed (mph): ", float(rf_data[14]), 20)
          alt.update(screen, "Alt (ft): ", float(rf_data[12]), 20)
              ...
              if(True):
                  with open("positions.kml", "w") as pos:
                   ...
                   xmlns:gx="http://www.google.com/kml/ext/2.2"><Placemark>
                                  ...
                                  </Placemark></kml>""" % (rf_data[16], rf_data[15], 0, rf_data[16], rf_data[15], 0))
              else:
                  print("INVALID GPS")
